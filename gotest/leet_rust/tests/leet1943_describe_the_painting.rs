...
mod _describe_the_painting {
    ...
    // line sweep, same as prefix sum, just that the array is not continuous
    use std::collections::HashMap;
    impl Solution {
        pub fn split_painting(segments: Vec<Vec<i32>>) -> Vec<Vec<i64>> {
            let mut ans = Vec::new();
            let mut map = HashMap::new();
            for s in segments {  // for each segment, add at start, sub at end
                *map.entry(s[0]).or_insert(0) += s[2] as i64;
                *map.entry(s[1]).or_insert(0) -= s[2] as i64;
            }
            let mut keys = map.keys().map(|&s| s).collect::<Vec<_>>(); // sort keys
            keys.sort();
            let mut sum = *map.get(&keys[0]).unwrap(); // this equals to prefix sum
            for i in 1..keys.len() {
                if sum > 0 {
                    ans.push(vec![keys[i-1] as i64, keys[i] as i64, sum]);
                }
                sum += *map.get(&keys[i]).unwrap();
            }
            return ans;
        }
    }
    ...
    fn test() {
        let testcases = vec![
            Solution {
                segments: vec![vec![1,4,5],vec![4,7,7],vec![1,7,9]],
                ans: vec![vec![1,4,14],vec![4,7,16]],
            },
            Solution {
                segments: vec![vec![1,7,9],vec![6,8,15],vec![8,10,7]],
                ans: vec![vec![1,6,9],vec![6,7,24],vec![7,8,15],vec![8,10,7]],
            },
            Solution {
                segments: vec![vec![1,4,5],vec![1,4,7],vec![4,7,1],vec![4,7,11]],
                ans: vec![vec![1,4,12],vec![4,7,12]],
            },
            Solution {
                segments: vec![[4,16,12],[9,10,15],[18,19,13],[3,13,20],[12,16,3],[2,10,10],[3,11,4],[13,16,6]]
                    .iter().map(|s| s.to_vec()).collect(),
                ans: vec![[2,3,10],[3,4,34],[4,9,46],[9,10,61],[10,11,36],[11,12,32],[12,13,35],[13,16,21],[18,19,13]]
                    .iter().map(|s| s.to_vec()).collect(),
            },
        ];
        for i in testcases {
            let ans = Solution::split_painting(i.segments);
            println!("{:?}, {:?}", ans, i.ans);
        }
    } 
}