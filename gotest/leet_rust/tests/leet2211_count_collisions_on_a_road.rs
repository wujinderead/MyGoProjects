...
// 'L', 'R', or 'S' denoting whether the iᵗʰ car is moving towards the left, towards the
...
// After a collision, the cars involved can no longer move and will stay at the point where
// they collided. Other than that, cars cannot change their state or direction of motion.
...
mod _count_collisions_on_a_road {
    struct Solution{
        directions: String,
        ans: i32,
    }

    // another esay solution: https://leetcode.com/problems/count-collisions-on-a-road/discuss/1865557/Not-a-stupid-problem!
    /*
        ...
        pub fn count_collisions(directions: String) -> i32 {
            ...
            for c in directions.chars() {
                if stack.is_empty() || &c == stack.last().unwrap() {
                    stack.push(c); // empty or same char, just push
                } else if c == 'L' {
                    // stack XXXS, current L, one SL collide, become S
                    if stack.last().unwrap() == &'S' {
                        sum += 1;
                    } else {
                        // last is R, for example: stack RRR, current L
                        // it become S; with one RL collide, two RS collide
                        while Some(&'R') == stack.last() {
                            sum += 1;
                            stack.pop();
                        }
                        sum += 1;
                    ...
                    if stack.last().unwrap() == &'L' {
                        stack.push('S');  // stack L, just push, it become LS
                    } else {  // stack RRR, current S: three RS collide, become S
                        while Some(&'R') == stack.last() {
                            sum += 1;
                            stack.pop();
                        }
    ...
    fn test() {
        let testcases = vec![
            Solution {
                directions: "RLRSLL".to_string(),
                ans: 5,
            },
            Solution {
                directions: "LLRR".to_string(),
                ans: 0,
            },
            Solution {
                directions: "SSRSSRLLRSLLRSRSSRLRRRRLLRRLSSRR".to_string(),
                ans: 20,
            },
        ];
        for i in testcases {
            let ans = Solution::count_collisions(i.directions);
            println!("{}, {}", ans, i.ans);
        }
    } 
}