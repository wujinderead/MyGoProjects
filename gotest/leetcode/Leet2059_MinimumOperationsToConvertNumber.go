...
// use BFS use check every values in [0,1000] are reachable
func minimumOperations(nums []int, start int, goal int) int {
	...
	for queue.Len() > 0 {
		step++
		leng := queue.Len()
		for i := 0; i < leng; i++ { // for the same layer
			cur := queue.Remove(queue.Front()).(int)
			for _, v := range nums {
				cand := [3]int{cur + v, cur - v, cur ^ v}
				for _, vv := range cand {
					if vv == goal {
						return step
					}
					if vv >= 0 && vv <= 1000 && !visited[vv] {
						visited[vv] = true
						queue.PushBack(vv)
					}
				}
...
func main() {
	for _, v := range []struct {
		...
		fmt.Println(minimumOperations(v.n, v.s, v.g), v.ans)
	}
}
