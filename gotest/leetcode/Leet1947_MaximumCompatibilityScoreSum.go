...
func maxCompatibilitySum(students [][]int, mentors [][]int) int {
	...
	// find all permutation of mentors
	permute(students, mentors, 0, m, n, &max)
	return max
}

func permute(st, me [][]int, start, m, n int, max *int) {
	if start == m-1 {
		sum := 0
		for i := 0; i < m; i++ {
			for j := 0; j < n; j++ {
	...
	for i := start; i < m; i++ {
		me[i], me[start] = me[start], me[i]
		permute(st, me, start+1, m, n, max)
		me[i], me[start] = me[start], me[i]
	}
}
...
func main() {
	for _, v := range []struct {
		...
		fmt.Println(maxCompatibilitySum(v.s, v.m), v.ans)
	}
}
