...
func countPaths(n int, roads [][]int) int {
	const mod = int(1e9 + 7)
	min := make([]int, n) // the minimal distance from vertex 0 to vertex i
	num := make([]int, n) // the number of paths with minimal distance from vertex 0 to vertex i
	num[0] = 1
	...
	for _, road := range roads {
		...
		out[u] = append(out[u], [2]int{v, t})
		out[v] = append(out[v], [2]int{u, t})
	}

	// set min[0]=1, won't influence minimal path
	h := &heaper{{0, 1}} // vertex and dist
	for h.Len() > 0 {
		tmp := heap.Pop(h).([2]int)
		v, dist := tmp[0], tmp[1] // current vertex with minimal path
		if min[v] > 0 {           // skip visited vertex
			continue
		}
		min[v] = dist                     // vertex 0 -> i, with minimal path = dist
		for _, neighbor := range out[v] { // for current vertex's neighbor
			vv, t := neighbor[0], neighbor[1]
			if min[vv] == 0 { // push unvisited vertex to heap
				heap.Push(h, [2]int{vv, dist + t})
			} else if min[vv] > 0 && min[vv]+t == dist { // neighbor is visited and from neighbor to current is minimal path
				num[v] = (num[v] + num[vv]) % mod
			}
		}
		if v == n-1 {
			return num[v]
