...
//     preparing it immediately at time 1, and finishes at time 3, so the waiting time of
//     the first customer is 3 - 1 = 2.
...
//     preparing it at time 8, and finishes at time 11, so the waiting time of the third
//     customer is 11 - 4 = 7.
...
//     preparing it immediately at time 5, and finishes at time 7, so the waiting time of
//     the first customer is 7 - 5 = 2.
//     2) The second customer arrives at time 5, the chef takes his order and starts
//     preparing it at time 7, and finishes at time 11, so the waiting time of the second
//     customer is 11 - 5 = 6.
//     3) The third customer arrives at time 10, the chef takes his order and starts
//     preparing it at time 11, and finishes at time 14, so the waiting time of the third
//     customer is 14 - 10 = 4.
//     4) The fourth customer arrives at time 20, the chef takes his order and starts
//     preparing it immediately at time 20, and finishes at time 21, so the waiting time
//     of the fourth customer is 21 - 20 = 1.
...
func averageWaitingTime(customers [][]int) float64 {
	...
	for _, v := range customers {
		if cur <= v[0] { // current customer is served immediately
			all += v[1]
			cur = v[0] + v[1]
		} else {
			all += cur - v[0] + v[1]
	...
	return float64(all) / float64((len(customers)))
}
...
func main() {
	for _, v := range []struct {
		...
		fmt.Println(averageWaitingTime(v.c), v.ans)
	}
}
