...
//     becomes 3. The next move Alice will take the pile with value = -9 and also lose.
...
func stoneGameIII(stones []int) string {
	// let F(i) be the max score the first taker can get for stones[i:]
	// then F(i) has three candidates, we want the max:
	//   stone[i] + sum(stones[i+1:])-F(i+1)
	//   stone[i] + stone[i+1] + sum(stones[i+2:])-F(i+2)
	//   stone[i] + stone[i+1] + stone[i+2] + sum(stones[i+3:])-F(i+3)
	...
	F[N] = 0
	for i:=N-1; i>=0; i-- {
		F[i] = stones[i] + suffixsum[i+1]-F[i+1]
		if i+1<len(stones) {
			F[i] = max(F[i], stones[i] + stones[i+1] + suffixsum[i+2]-F[i+2])
		}
		if i+2<len(stones) {
			F[i] = max(F[i], stones[i] + stones[i+1] + stones[i+2] + suffixsum[i+3]-F[i+3])
		}
	}
	if F[0]>suffixsum[0]-F[0] {
		return "Alice"
	} else if F[0]<suffixsum[0]-F[0] {
		return "Bob"
	}
	return "Tie"
}
...
func main() {
    ...
    fmt.Println(stoneGameIII([]int{1, -1}))
    fmt.Println(stoneGameIII([]int{1, 2}))
    fmt.Println(stoneGameIII([]int{1}))
}