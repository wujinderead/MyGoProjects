...
func findAllPeople(n int, meetings [][]int, firstPerson int) []int {
	...
	root[firstPerson] = 0                      // initially, {0, firstPerson} are in the same set
	sort.Slice(meetings, func(i, j int) bool { // sort by time
	...
	for i < len(meetings) {
		ti := meetings[i][2]
		for j = i; j < len(meetings) && meetings[j][2] == ti; j++ { // the meetings at same time
			ra := getRoot(root, meetings[j][0])
			rb := getRoot(root, meetings[j][1])
			if ra == 0 || rb == 0 { // if one person knows secret, set both person's root to 0; it means they both know now
				root[ra] = 0
				root[rb] = 0
			} else {
				root[ra] = rb // just connect 2 persons
			}
		}
		for j = i; j < len(meetings) && meetings[j][2] == ti; j++ {
			// for unknown persons in this batch, need reset their connections
			ra := getRoot(root, meetings[j][0])
			if ra != 0 {
				root[meetings[j][0]] = meetings[j][0]
				root[meetings[j][1]] = meetings[j][1]
			}
		}
		i = j // to meetings at next same time
	}

	// the persons with root 0 are those who know secret
	ans := make([]int, 0)
	for i = range root {
		if getRoot(root, i) == 0 {
			ans = append(ans, i)
...
func main() {
	for _, v := range []struct {
		...
		fmt.Println(findAllPeople(v.n, v.m, v.f), v.ans)
	}
}
