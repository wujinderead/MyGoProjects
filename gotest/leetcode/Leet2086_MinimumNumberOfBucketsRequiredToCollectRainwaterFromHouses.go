...
// The rainwater from a house at index i is collected if a bucket is placed at index i - 1 and/or index
// i + 1. A single bucket, if placed adjacent to two houses, can collect the rainwater from both houses.
...
// https://leetcode.com/problems/minimum-number-of-buckets-required-to-collect-rainwater-from-houses/discuss/1598919/JavaPython1-Liner-Solution
func minimumBuckets(street string) int {
	if street == "H" || (len(street) >= 2 && (street[:2] == "HH" || street[len(street)-2:] == "HH")) ||
		strings.Contains(street, "HHH") {
		return -1
	}
...
func main() {
	for _, v := range []struct {
		...
		fmt.Println(minimumBuckets(v.s), v.ans)
	}
}
