...
func mincostTickets(days []int, costs []int) int {
	// let c[i] be the total cost until days[i], then
	// c[i] = min( c[i-1]+cost1 , c[i-7]+cost7 , c[i-30]+cost30 )
	// base case: c[0]=0
	if len(days) == 0 {
		return 0
	}
	c := make([]int, days[len(days)-1]-days[0]+1)
	vind := 0
	for i := range c {
		if i == days[vind]-days[0] {
	...
	//fmt.Println(c)
	return c[days[len(days)-1]-days[0]]
}
