...
// operations i times. Remember that any index j can be picked at most once.
...
func canConvertString(s string, t string, k int) bool {
	...
	for i := range s {
		if s[i]==t[i] {
			continue   // ignore the same
		} 
		shift := (int(t[i])-int(s[i])+26) % 26
		if take[shift]*26+shift > k {
			return false
		}
		take[shift]++
	}
...
func main() {
	...
	fmt.Println(canConvertString("abc", "abcd", 1000), false)
	fmt.Println(canConvertString("iqssxdlb", "dyuqrwyr", 40), true)
}