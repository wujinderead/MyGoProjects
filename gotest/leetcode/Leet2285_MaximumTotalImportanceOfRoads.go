...
// from 0 to n - 1.
// You are also given a 2D integer array roads where roads[i] = [ai, bi] denotes that there exists
...
func maximumImportance(n int, roads [][]int) int64 {
	count := make([]int, n)
	for _, v := range roads { // count edge
		count[v[0]]++
		count[v[1]]++
	}
	max := 0
	mapp := make(map[int]int)
	for _, v := range count { // frequency of each count
		mapp[v] = mapp[v] + 1
		if v > max {
			max = v
		}
	...
	ans := 0
	for i := max; i > 0; i-- { // multiply with count
		v := mapp[i]
		if v > 0 {
			ans += i * (n + n - v + 1) * v / 2
			n = n - v
...
func main() {
	for _, v := range []struct {
		...
		{5, [][]int{{0, 1}, {1, 2}, {2, 3}, {0, 2}, {1, 3}, {2, 4}}, 43},
		{5, [][]int{{0, 3}, {2, 4}, {1, 3}}, 20},
	} {
		fmt.Println(maximumImportance(v.n, v.roads))
	}
}
