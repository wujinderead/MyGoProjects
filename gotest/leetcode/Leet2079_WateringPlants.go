...
// ith plant needs, and an integer capacity representing the watering can capacity, return the
...
func wateringPlants(plants []int, capacity int) int {
	...
	steps := 0
	for i := range plants {
		if cur >= plants[i] { // can water current plant, water it
			cur = cur - plants[i]
			steps++
		} else { // go back refill
...
func main() {
	for _, v := range []struct {
		...
		fmt.Println(wateringPlants(v.p, v.c), v.ans)
	}
}
