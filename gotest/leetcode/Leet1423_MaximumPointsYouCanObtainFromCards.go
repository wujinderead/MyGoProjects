...
//     the rightmost card first will maximize your total score. The optimal strategy
...
func maxScore(cardPoints []int, k int) int {
    // no matter what the order is, we finally choose the most left i nums, most right k-i nums.
    max := 0
    for i:=0; i<k; i++ {
    	max += cardPoints[i]
	}
	...
	for i, j := k-1, len(cardPoints)-1; i>=0; i, j = i-1, j-1 {
		cand = cand-cardPoints[i]+cardPoints[j]
		if cand>max {
			max = cand
		}
	}
