...
//   Output: 3
//   Explanation: Let's see what happened in the first three days. x means flower bloomed and _ means flower 
//     didn't bloom in the garden.
...
//     the last three flowers that bloomed because they are not adjacent. After day 12: [x, x, x, x, x, x, x]
...
func minDays(bloomDay []int, m int, k int) int {
    ...
    l, r := 0, int(1e9)
    for l<r {                  // O(logn) for binary search
    	mid := (l+r)/2
    	count := 0
    	consecutive := 0
    	for _, v := range bloomDay {   // O(n) to find count
    		if mid>=v {
    			consecutive++
    		} else {
    			count += consecutive/k
    	...
    	count += consecutive/k
    	if count>=m {        // find the minimal value that 'can', we set r=mid and l=mid+1
    		r = mid   		
    	} else {
    		l = mid+1
    	}
...
func main() {
	...
	fmt.Println(minDays([]int{1000000000,1000000000}, 1, 1), 1000000000)
}