...
// where m is the median of the array. If |arr[i] - m| == |arr[j] - m|, then arr[i] is said 
// to be stronger than arr[j] if arr[i] > arr[j]. 
// Return a list of the strongest k values in the array. return the answer in any arbitrary order. 
// Median is the middle value in an ordered integer list. More formally, if the length of the list 
// is n, the median is the element in position ((n - 1) / 2) in the sorted list (0-indexed). 
// For arr = [6, -3, 7, 2, 11], n = 5 and the median is obtained by sorting the 
// array arr = [-3, 2, 6, 7, 11] and the median is arr[m] where m = ((5 - 1) / 2) = 2. The median is 6. 
...
func getStrongest(arr []int, k int) []int {
    ...
    mid := arr[(len(arr)-1)/2]
    for i:=0; i<k; i++ {
    	a, b := abs(arr[l], mid), abs(arr[r], mid)
    	if a>b {
    		ans[i] = arr[l]
    		l++
    	} else {
...
func main() {
	...
	fmt.Println(getStrongest([]int{1,1,3,5,5}, 2))
	fmt.Println(getStrongest([]int{6,7,11,7,6,8}, 5))
	fmt.Println(getStrongest([]int{6,-3,7,2,11}, 3))
	fmt.Println(getStrongest([]int{-7,22,17,3}, 2))
	fmt.Println(getStrongest([]int{-7,22,17,3}, 4))
}