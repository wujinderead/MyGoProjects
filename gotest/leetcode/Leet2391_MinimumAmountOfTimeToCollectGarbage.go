...
// garbage at the iᵗʰ house. garbage[i] consists only of the characters 'M', 'P' and 'G' representing
// one unit of metal, paper and glass garbage respectively. Picking up one unit of any type of garbage
// takes 1 minute. You are also given a 0-indexed integer array travel where travel[i] is the number
...
// find last occurrence of G P M
func garbageCollection(garbage []string, travel []int) int {
	var hasG, hasM, hasP bool
	ans := 0
...
func main() {
	for _, v := range []struct {
		...
		fmt.Println(garbageCollection(v.garbage, v.travel), v.ans)
	}
}
