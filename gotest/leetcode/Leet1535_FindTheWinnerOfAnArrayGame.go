...
// an integer wins k consecutive rounds. Return the integer which will win the game.
...
func getWinner(arr []int, k int) int {
	...
	for r < len(arr) {
		if arr[l] > arr[r] {
			lwin++            // l larger, increment lwin, move r right
			if lwin==k {
				return arr[l]
			}
			r++
			rwin = 0
		} else {              // r larger, increment rwin, move l to r+1, switch l r
			rwin++
			if rwin==k {
				return arr[r]
			}
	...
	return arr[l]   // the largest number can win infinite times
}
...
func main() {
	for _, v := range []struct{arr []int; k, ans int} {
		...
		fmt.Println(getWinner(v.arr, v.k), v.ans)
	}
}